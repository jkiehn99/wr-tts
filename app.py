...
def run_local_tts(text: str, out_file: Path, voice: str, rate: int) -> None:
...
def run_tts(text: str, out_file: Path, state: JobState) -> None:
    if state.tts_provider == "proxy":
        ...
        return
    run_local_tts(text, out_file, state.voice, state.rate)
...
<style>
body{margin:0;background:#0f172a;color:#e5e7eb;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.wrap{max-width:900px;margin:0 auto;padding:40px 20px}.card{position:relative;background:#111827;border:1px solid #374151;border-radius:18px;padding:24px;box-shadow:0 20px 80px #0006}.top{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}h1{margin:0 0 8px;font-size:30px}.sub{color:#94a3b8;margin-bottom:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.full{grid-column:1/-1}label{display:block;color:#cbd5e1;margin:10px 0 6px}input,select,button,textarea{box-sizing:border-box;width:100%;border-radius:12px;border:1px solid #475569;background:#020617;color:#e5e7eb;padding:12px;font-size:15px}textarea{min-height:86px;font-family:ui-monospace,Menlo,monospace}button{margin-top:18px;background:#2563eb;border-color:#2563eb;font-weight:700;cursor:pointer}button.secondary{background:#16a34a;border-color:#16a34a}button.ghost{width:auto;margin-top:0;background:#1f2937;border-color:#475569;padding:10px 14px}button:disabled{opacity:.55;cursor:not-allowed}.bar{height:14px;background:#020617;border-radius:999px;overflow:hidden;border:1px solid #334155;margin-top:18px}.fill{height:100%;width:0;background:linear-gradient(90deg,#38bdf8,#22c55e)}pre{white-space:pre-wrap;background:#020617;border:1px solid #334155;border-radius:12px;padding:14px;color:#cbd5e1}.download{display:none;margin-top:16px;color:#93c5fd}.preview{display:none;margin-top:16px}.preview audio{width:100%;margin-top:8px}.hint{font-size:13px;color:#94a3b8;line-height:1.6}.settings{display:none;margin:0 0 18px;padding:16px;border:1px solid #334155;border-radius:14px;background:#020617}.settings.open{display:block}.settings .row{display:grid;grid-template-columns:1fr 1fr;gap:12px}.muted{color:#94a3b8;font-size:13px}.voiceTools{display:grid;grid-template-columns:1fr 160px;gap:8px;margin-top:8px}@media(max-width:700px){.grid,.settings .row,.voiceTools{grid-template-columns:1fr}.top{display:block}.top button{margin:10px 0 0}}
</style>
</head>
<body><div class="wrap"><div class="card">
<div class="top"><div><h1>小秋有声书生成器</h1>
<div class="sub">上传 txt / md / epub / pdf / docx / mobi / azw3 / zip，先试听，满意后再生成整本 MP3。</div></div><button id="settingsToggle" class="ghost" type="button">TTS 设置</button></div>
<div id="settingsBox" class="settings">
 ...
 <div class="row">
  <div><label>代理模型</label><input id="proxyModel" name="proxy_model" value="tts-1" placeholder="tts-1 / eleven_multilingual_v2 / speech-2.6-turbo" /></div>
  <div><label>代理 voice_id</label><input id="proxyVoiceId" placeholder="留空则使用音色下拉值；Google 填 voice name；ElevenLabs/MiniMax 填专用 voice_id" /></div>
 </div>
 <label>代理 URL</label><input id="proxyUrl" name="proxy_url" placeholder="OpenAI: https://api.openai.com/v1；Google: https://texttospeech.googleapis.com/v1；ElevenLabs: https://api.elevenlabs.io/v1；MiniMax: https://api-uw.minimax.io/v1" />
 <div class="row"><div><label>代理 API Key</label><input id="proxyApiKey" name="proxy_api_key" type="password" placeholder="保存到本地 settings.json，不显示在任务状态里" /></div><div><label>MiniMax group_id</label><input id="proxyGroupId" placeholder="MiniMax 可选 group_id" /></div></div>
 ...
 <button id="saveSettings" class="ghost" type="button">保存设置</button> <button id="testProxy" class="ghost" type="button">测试代理</button><span id="settingsMsg" class="muted"></span>
</div>
...
<div><label>音色选择</label><select id="voiceSelect" name="voice"><option value="zh-CN-XiaoqiuNeural">晓秋 - 中文普通话</option></select><div class="voiceTools"><input id="voiceSearch" placeholder="搜索晓秋、中文、香港或英语音色" /><select id="voiceLocale"><option value="">全部语言</option></select></div></div>
...
<div class="full"><label>说明</label><div class="hint">先生成一小段试听，确认音色、语速和效果满意后，再点击生成整本。试听任务若不继续生成，约 30 分钟后自动删除；整本下载后也会自动删除服务器文件。建议每段 1500–1800 字。</div></div>
...
<script>
const form=document.getElementById('form'),btn=document.getElementById('btn'),log=document.getElementById('log'),fill=document.getElementById('fill'),download=document.getElementById('download'),previewBox=document.getElementById('previewBox'),previewAudio=document.getElementById('previewAudio'),startFull=document.getElementById('startFull'),voiceSelect=document.getElementById('voiceSelect'),voiceSearch=document.getElementById('voiceSearch'),voiceLocale=document.getElementById('voiceLocale'),settingsToggle=document.getElementById('settingsToggle'),settingsBox=document.getElementById('settingsBox'),ttsProvider=document.getElementById('ttsProvider'),proxyType=document.getElementById('proxyType'),proxyUrl=document.getElementById('proxyUrl'),proxyApiKey=document.getElementById('proxyApiKey'),proxyModel=document.getElementById('proxyModel'),proxyVoiceId=document.getElementById('proxyVoiceId'),proxyGroupId=document.getElementById('proxyGroupId'),customHeaders=document.getElementById('customHeaders'),customBodyTemplate=document.getElementById('customBodyTemplate'),customAudioField=document.getElementById('customAudioField'),saveSettings=document.getElementById('saveSettings'),testProxy=document.getElementById('testProxy'),settingsMsg=document.getElementById('settingsMsg');
...
function settingsPayload(){return {provider:ttsProvider.value,proxy_type:proxyType.value,proxy_url:proxyUrl.value,proxy_api_key:proxyApiKey.value,proxy_model:proxyModel.value,proxy_voice_id:proxyVoiceId.value,proxy_group_id:proxyGroupId.value,custom_headers:customHeaders.value,custom_body_template:customBodyTemplate.value,custom_audio_field:customAudioField.value}}
function renderVoices(){
 const q=(voiceSearch.value||'').toLowerCase(), loc=voiceLocale.value, current=voiceSelect.value;
 voiceSelect.innerHTML='';
 for(const v of allVoices){
  const hay=((v.name||'')+' '+(v.display||'')).toLowerCase();
  const locale=v.locale||((v.name||'').split('-').slice(0,2).join('-'));
  if(q && !hay.includes(q))continue; if(loc && locale!==loc)continue;
...
function localePriority(code,label){
 const text=(code+' '+label).toLowerCase();
 if(code==='zh-CN')return 0; if(code==='zh-HK')return 1; if(code==='zh-TW')return 2;
 if(text.includes('中文')||code.startsWith('zh-'))return 3;
 if(code==='en-US')return 10; if(code==='en-GB')return 11; if(code.startsWith('en-'))return 12;
 if(code.startsWith('ja-'))return 20; if(code.startsWith('ko-'))return 21;
 if(code.startsWith('fr-'))return 30; if(code.startsWith('de-'))return 31; if(code.startsWith('es-'))return 32;
 return 100;
}
async function loadVoices(){
 try{const r=await fetch('/api/voices'); const j=await r.json(); allVoices=j.voices||[]; const localeMap=new Map(); for(const v of allVoices){const code=v.locale||((v.name||'').split('-').slice(0,2).join('-')); if(code&&!localeMap.has(code))localeMap.set(code,v.locale_label||code);} const locales=[...localeMap.entries()].sort((a,b)=>localePriority(a[0],a[1])-localePriority(b[0],b[1])||a[1].localeCompare(b[1])); voiceLocale.innerHTML='<option value="">全部语言</option>'+locales.map(([code,label])=>`<option value="${code}">${label}</option>`).join(''); if(j.default){voiceSelect.value=j.default;} renderVoices();}
 catch(e){show('音色列表加载失败：'+e.message)}
}
async function loadSettings(){
 try{const r=await fetch('/api/settings'); const s=await r.json(); ttsProvider.value=s.provider||'local'; proxyType.value=s.proxy_type||'openai'; proxyUrl.value=s.proxy_url||''; proxyModel.value=s.proxy_model||'tts-1'; proxyVoiceId.value=s.proxy_voice_id||''; proxyGroupId.value=s.proxy_group_id||''; customHeaders.value=s.custom_headers||''; customBodyTemplate.value=s.custom_body_template||''; customAudioField.value=s.custom_audio_field||'';}
 catch(e){settingsMsg.textContent='设置加载失败'}
}
voiceSearch.addEventListener('input',renderVoices); voiceLocale.addEventListener('change',renderVoices);
...
async function poll(id){
 ...
 show({状态:j.status,后端:j.tts_provider,模板:j.proxy_type,消息:j.message,进度:`${j.done_chunks}/${j.total_chunks}`,字符数:j.chars,错误:j.error||''});
